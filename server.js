  ...
  try {
    ...
    if (userExists) {
      ...
      if (passwordMatched) {
        ...
        res.cookie("token", accessToken, {maxAge: utility.getNextYearEpochTime(), sameSite: 'none', secure: true, domain: 'go-swiftcart.netlify.app'});
        res.cookie("refresh_token", refreshToken, {maxAge: utility.getNextYearEpochTime(), sameSite: 'none', secure: true, domain: 'go-swiftcart.netlify.app'});
        return res.send();
      } else {
